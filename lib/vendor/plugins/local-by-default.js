    ...
    case 'selectors':
      ...
      newNodes = node.nodes.map(function (n) {
        ...
        if (typeof resultingGlobal === 'undefined') {
          ...
          throw new Error('Inconsistent rule global/local result in rule "' + _cssSelectorTokenizer2.default.stringify(node) + '" (multiple selectors must result in the same mode for the rule)');
        }
...
function localizeAnimationShorthandDeclValues(valuesNode, decl, context) {
  ...
  decl.value = _cssSelectorTokenizer2.default.stringifyValues(newValuesNode);
}
...
function localizeDecl(decl, context) {
  var valuesNode = _cssSelectorTokenizer2.default.parseValues(decl.value);
...
var process = function process(css) {
  ...
  css.walkRules(function (rule) {
    ...
    if (pureMode && context.hasPureGlobals) {
      throw rule.error('Selector "' + _cssSelectorTokenizer2.default.stringify(selector) + '" is not pure ' + '(pure selectors must contain at least one local class or id)');
    }
